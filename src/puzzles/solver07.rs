...
pub fn solve() {
    ...
    let mut mapped_cards1: Vec<(u32, u32)> = txt
        .lines()
        .map(|line| {
    ...
    let mut mapped_cards2: Vec<(u32, u32)> = txt
        .lines()
        .map(|line| {
            let splits = line.split_whitespace().collect::<Vec<&str>>();
            ...
            for j in 0..cards2.len() - 1 {
                let mut st = get_strength(
                    splits[0]
                        .replace('J', cards2[j].to_string().as_str())
                        .as_str(),
                );

                for card in splits[0].chars() {
                    st =
                        13 * st + (13 - cards2.into_iter().position(|c| c == card).unwrap() as u32);
                }
