...
pub fn solve() {
    ...
    for line in lines {
        ...
        let caps0: Vec<u32> = re0.find_iter(line).map(|x| parse_line(x.as_str())).collect();
        let caps1: Vec<u32> = re1.find_iter(line).map(|x| parse_line(x.as_str())).collect();
        let caps2: Vec<u32> = re2.find_iter(line.chars().rev().collect::<String>().as_str()).map(|x| parse_line(x.as_str().chars().rev().collect::<String>().as_str())).collect();
