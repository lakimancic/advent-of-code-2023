...
fn dfs(
    ...
    if part1 && mat[pos.0 as usize][pos.1 as usize] != '.' {
        ...
        let res = dfs(mat, n, m, vis, new_pos, true);
    ...
    for (di, dj) in DIRS {
        ...
        let new_res = dfs(mat, n, m, vis, np, part1);
        if new_res != -1 {
...
pub fn solve() {
    ...
    println!(
        "Part 1 solution is: {}",
        dfs(&mat, n, m, &mut vis, (0, 1), true)
    );
    println!(
        "Part 2 solution is: {}",
        dfs(&mat, n, m, &mut vis, (0, 1), false)
    );
}
